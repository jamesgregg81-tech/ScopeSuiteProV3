...
def try_get_id(ser: serial.Serial):
    """
    Returns tuple(ok, ack_code, ident_or_none)
    """
    print("\n>> ID")
    try:
        ser.reset_input_buffer()
...
def ensure_remote_ready(ser: serial.Serial) -> None:
    """
    Some units/states reply ACK 1 to ID until nudged with GR/GL/GR.
    This function tries a few safe commands and doesn't fail hard
    unless the port is completely dead.
    """
    # Try GR first without clearing input too aggressively.
    for cmd in ["GR", "GL", "GR"]:
        try:
...
def detect_scope(preferred_port: str):
    ...
    for port_name in ordered_ports:
        for baud in PREFERRED_BAUDS:
            ...
            # Weak positive match: got an ACK digit from the instrument.
            # This still proves we are on the right port/baud and the instrument heard us.
...
def connect_scope(preferred_port: str):
    # Try preferred port directly first.
    if preferred_port:
        for baud in PREFERRED_BAUDS:
            try:
...
def write_csv(csv_path, row):
    headers = [
        "vrms_v", "irms_a", "real_power_w", "apparent_power_va", "reactive_power_var",
...
def main():
    ...
    try:
        ...
        wf_a = None
        try:
            wf_a = query_waveform(ser, "10")  # Voltage
        except RuntimeError as e:
            print(f"Channel A not available: {e}")

        wf_b = query_waveform(ser, "20")  # Current
        ...
        # =========================
        # If voltage exists → full analysis
        # =========================
        if wf_a is not None:
        ...
        if power is not None:
            ...
            irms = float(np.sqrt(np.mean(np.asarray(wf_b["y"]) ** 2)))
            with open(summary_txt, "w", encoding="utf-8") as f:
                f.write(
                    f"Neutral Current Only\n\n"
                    f"Irms: {irms:.3f} A\n"
                    f"Freq: {dom_b:.2f} Hz\n"
                    f"THD: {thd_b:.4f}\n"
                )
