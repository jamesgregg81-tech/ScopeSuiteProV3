...
DEFAULT_BAUD = 1200
FAST_BAUD = 9600
INITIAL_BAUDRATE = DEFAULT_BAUD
WORK_BAUDRATE = FAST_BAUD
...
# Fixed channel assignment for this setup:
# Channel A = Voltage, Channel B = Current
...
# Line-frequency handling. Set to 60.0 or 50.0 to force the nearest FFT bin
# used for line-frequency, phase, and harmonic calculations. Set to None to
# auto-pick whichever of 50/60 Hz has the stronger nearby FFT amplitude.
EXPECTED_LINE_FREQUENCY_HZ = 60.0
LINE_FREQUENCY_CANDIDATES_HZ = (50.0, 60.0)

# Power-factor correction calculator settings for capacitor sizing in the reports.
PF_CORRECTION_START_POWER_FACTOR = 0.816
...
SERIAL_POST_BAUD_SWITCH_DELAY_S = 0.75
...
def select_replay_screen(ser: serial.Serial, index: int):
    ...
    if code != 0:
        if code == 2:
            ...
            if nr_screens > 0 and screen_index == index:
                print(f"RP {index} returned ACK=2, but replay index {index} is already selected; continuing.")
                return
...
def interpolate_fft_peak(freq, amp, idx):
    if idx <= 0 or idx >= len(amp) - 1:
        return float(freq[idx]), float(amp[idx]), float(idx)

    left = float(amp[idx - 1])
    center = float(amp[idx])
    right = float(amp[idx + 1])
    denom = left - 2.0 * center + right
    if abs(denom) < 1e-30:
        return float(freq[idx]), center, float(idx)

    offset = 0.5 * (left - right) / denom
    offset = max(-1.0, min(1.0, offset))
    bin_width = float(freq[1] - freq[0]) if len(freq) > 1 else 0.0
    peak_freq = float(freq[idx] + offset * bin_width)
    peak_amp = float(center - 0.25 * (left - right) * offset)
    return peak_freq, peak_amp, float(idx) + offset


def expected_line_frequency_hz(freq, amp):
    candidates = []
    if EXPECTED_LINE_FREQUENCY_HZ is not None:
        candidates.append(float(EXPECTED_LINE_FREQUENCY_HZ))
    else:
        candidates.extend(float(f) for f in LINE_FREQUENCY_CANDIDATES_HZ)

    valid = [f for f in candidates if len(freq) > 1 and freq[1] <= f <= freq[-1]]
    if not valid:
        return None
    ...
    def nearest_amp(candidate):
        idx = int(np.argmin(np.abs(freq - candidate)))
        return float(amp[idx])

    return max(valid, key=nearest_amp)
...
def dominant_frequency(freq, amp):
    ...
    peak_freq, peak_amp, _ = interpolate_fft_peak(freq, amp, idx)
    return peak_freq, peak_amp, int(idx)


def line_frequency_peak(freq, amp):
    expected = expected_line_frequency_hz(freq, amp)
    if expected is None:
        return dominant_frequency(freq, amp)

    idx = int(np.argmin(np.abs(freq - expected)))
    _peak_freq, peak_amp, _ = interpolate_fft_peak(freq, amp, idx)
    return float(expected), peak_amp, idx


def thd_from_fft(freq, amp, max_harmonic=15):
    f1, a1, _ = line_frequency_peak(freq, amp)
    ...
    for n in range(2, max_harmonic + 1):
        ...
        _harmonic_freq, an, _ = interpolate_fft_peak(freq, amp, idx)
        harmonics[n] = an
        sum_sq += an * an
...
def capacitor_correction(real_power_w, present_pf, target_pf, voltage_rms, line_frequency_hz):
    if not all(np.isfinite(v) for v in [real_power_w, present_pf, target_pf, voltage_rms, line_frequency_hz]):
        return None
    if real_power_w <= 0 or voltage_rms <= 0 or line_frequency_hz <= 0:
        return None

    present_pf = abs(float(present_pf))
    target_pf = abs(float(target_pf))
    if present_pf <= 0 or present_pf >= target_pf or target_pf >= 1.0:
        return None

    phi_now = math.acos(max(-1.0, min(1.0, present_pf)))
    phi_target = math.acos(max(-1.0, min(1.0, target_pf)))
    qc_var = float(real_power_w * (math.tan(phi_now) - math.tan(phi_target)))
    if qc_var <= 0:
        return None

    capacitance_f = qc_var / (2.0 * math.pi * line_frequency_hz * voltage_rms ** 2)
    return {
        "target_power_factor": target_pf,
        "correction_kvar": qc_var / 1000.0,
...
def analyze_voltage_current(voltage_samples, current_samples, dt, current_scale_a_per_v):
    ...
    v_amp = (2.0 / np.sum(window)) * np.abs(V)
    fundamental_hz, _fundamental_amp, k = line_frequency_peak(freqs, v_amp)
    ...
    pf_correction = capacitor_correction(
        real_power_w,
        PF_CORRECTION_START_POWER_FACTOR,
        TARGET_POWER_FACTOR,
        vrms,
        fundamental_hz,
    )
...
def normalize_channel_metadata(wf_a, wf_b):
    """
    Force the display/analysis metadata to match the user's wiring convention:
    Channel A = Voltage, Channel B = Current.
    """
    wf_a = dict(wf_a)
    wf_b = dict(wf_b)
...
def save_waveform_plot(path, frame_name, wf_a, wf_b):
    ...
    plt.plot(wf_a["x"], wf_a["y"], label=f"{wf_a['display_label']} ({wf_a['display_unit']})")
    plt.plot(wf_b["x"], wf_b["y"], label=f"{wf_b['display_label']} ({wf_b['display_unit']})")
...
def save_fft_plot(path, frame_name, freq_a, amp_a, freq_b, amp_b, unit_a, unit_b):
    plt.figure(figsize=(10, 6))
    plt.plot(freq_a, amp_a, label=f"{CHANNEL_A_LABEL} FFT ({CHANNEL_A_DISPLAY_UNIT})")
    plt.plot(freq_b, amp_b, label=f"{CHANNEL_B_LABEL} FFT ({CHANNEL_B_DISPLAY_UNIT})")
    plt.xlim(left=0)
...
def frame_summary_text(frame_name, power, wf_a, dom_a, ampdom_a, wf_b, dom_b, ampdom_b, thd_a, thd_b):
    lines = [
        ...
        "",
        "Channel A Voltage",
        f"Unit: {wf_a['display_unit']}",
        f"RMS: {power['vrms_v']:.6f}",
        f"Dominant frequency: {dom_a:.6f} Hz",
        f"Dominant amplitude: {ampdom_a:.6f} {wf_a['display_unit']}",
        f"THD estimate: {thd_a:.6f}",
        "",
        "Channel B Current",
        f"Unit: {wf_b['display_unit']}",
        f"RMS: {power['irms_a']:.6f}",
        f"Dominant frequency: {dom_b:.6f} Hz",
        f"Dominant amplitude: {ampdom_b:.6f} {wf_b['display_unit']}",
    ...
    pf_correction = power.get("pf_correction")
    if pf_correction:
        lines += [
            "",
            "Capacitor Correction",
            f"Raise PF from {PF_CORRECTION_START_POWER_FACTOR:.3f} to {TARGET_POWER_FACTOR:.3f}",
            f"Required capacitor: {pf_correction['capacitance_uf']:.2f} uF at {power['vrms_v']:.2f} V, {power['fundamental_hz']:.2f} Hz",
            f"Required correction: {pf_correction['correction_kvar']:.3f} kVAR",
        ]
    return "\n".join(lines)
...
def write_csv(csv_path, rows):
    headers = [
        ...
        "pf_correction_from", "pf_correction_to",
        "pf_correction_kvar", "pf_correction_uf",
...
def worst_case_summary(rows):
    ...
    high_pf_correction = best_row("pf_correction_kvar")
    if high_pf_correction:
        lines += [
            f"Largest capacitor correction frame: {high_pf_correction['frame_name']}  "
            f"{high_pf_correction['pf_correction_kvar']:.3f} kVAR / {high_pf_correction['pf_correction_uf']:.2f} uF",
            "",
        ]
    return "\n".join(lines)
...
def global_summary_text(ident, rp_count, rows):
    ...
    pf_correction_kvar_vals = np.array([r["pf_correction_kvar"] for r in rows], dtype=float)
    pf_correction_uf_vals = np.array([r["pf_correction_uf"] for r in rows], dtype=float)
    ...
    lines = [
        ...
        f"Capacitor correction {PF_CORRECTION_START_POWER_FACTOR:.3f}->{TARGET_POWER_FACTOR:.3f} PF, kVAR mean/max: {np.nanmean(pf_correction_kvar_vals):.3f} / {np.nanmax(pf_correction_kvar_vals):.3f}",
        f"Capacitor correction {PF_CORRECTION_START_POWER_FACTOR:.3f}->{TARGET_POWER_FACTOR:.3f} PF, uF mean/max: {np.nanmean(pf_correction_uf_vals):.2f} / {np.nanmax(pf_correction_uf_vals):.2f}",
...
def export_replay_reports(ser, ident):
    ...
    for idx in replay_indices:
        ...
        wf_a, wf_b = normalize_channel_metadata(wf_a, wf_b)
        freq_a, amp_a, _ = compute_fft(wf_a["x"], wf_a["y"])
        freq_b, amp_b, _ = compute_fft(wf_b["x"], wf_b["y"])
        dom_a, ampdom_a, _ = line_frequency_peak(freq_a, amp_a)
        dom_b, ampdom_b, _ = line_frequency_peak(freq_b, amp_b)
        ...
        save_fft_plot(fft_png, frame_name, freq_a, amp_a, freq_b, amp_b, wf_a["display_unit"], wf_b["display_unit"])
        with open(summary_txt, "w", encoding="utf-8") as f:
            f.write(frame_summary_text(frame_name, power, wf_a, dom_a, ampdom_a, wf_b, dom_b, ampdom_b, thd_a, thd_b))

        pf_correction = power.get("pf_correction") or {}
        ...
        rows.append({
            ...
            "pf_correction_from": PF_CORRECTION_START_POWER_FACTOR,
            "pf_correction_to": TARGET_POWER_FACTOR,
            "pf_correction_kvar": pf_correction.get("correction_kvar", float("nan")),
            "pf_correction_uf": pf_correction.get("capacitance_uf", float("nan")),
    ...
    return {
        ...
        "csv_path": csv_path,
        "trend_png": trend_png,
        "harmonic_trend_png": harmonic_trend_png,
        "global_summary_txt": global_summary_txt,
        "rows": rows,
        "nr_screens": nr_screens,
    }


def main():
    print("Fluke ScopeMeter Analyzer EXE Starting...")
    print("Fluke Final Replay Field Tool starting...")
    print("Fixed channel map: Channel A = Voltage, Channel B = Current")

    ser = None

    try:
        ser, ident = connect_scope(PORT)
        export_replay_reports(ser, ident)
    finally:
        if ser is not None:
            try:
                ser.close()
