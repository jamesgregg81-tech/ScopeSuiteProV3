...
def collect_waveform_samples(report_dir):
    ...
    for idx in range(n):
        ...
        if idx < len(a_rows):
            try:
                t = float(a_rows[idx]["time"])
                v = float(a_rows[idx]["channel_a_voltage"])
            except (KeyError, TypeError, ValueError):
                pass
        if idx < len(b_rows):
            try:
                if not np.isfinite(t):
                    t = float(b_rows[idx]["time"])
                i = float(b_rows[idx]["channel_b_current"])
            except (KeyError, TypeError, ValueError):
                pass
        if np.isfinite(t):
            samples.append({
...
<div class="warn">Visual approximation only -- numeric waveform data unavailable.</div>
