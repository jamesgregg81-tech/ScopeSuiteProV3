...
from .config import CURRENT_SCALE_A_PER_V, TIMEBASE_CORRECTION
...
def stitch_replay_waveform_csvs(export_dir, rows, log=None):
    ...
    row_by_frame = {r["frame_name"]: r for r in rows}
    max_current = max(rows, key=lambda r: r["irms_a"]) if rows else None
    lowest_pf = min(rows, key=lambda r: r["power_factor"]) if rows else None
    ...
    if max_current:
        log(f"Stitched Replay View: detected max current frame={max_current['frame_name']} Irms={max_current['irms_a']:.6g}")
    if lowest_pf:
        log(f"Stitched Replay View: detected lowest PF frame={lowest_pf['frame_name']} PF={lowest_pf['power_factor']:.6g}")
...
def save_stitched_replay_plot(stitched_csv, stitched_png, frame_ranges, row_by_frame, log=None):
    ...
    if row_by_frame:
        anomalies = [
            ("min Vrms", min(row_by_frame.values(), key=lambda r: r["vrms_v"]), "#2f6fed"),
            ("max Vrms", max(row_by_frame.values(), key=lambda r: r["vrms_v"]), "#2f6fed"),
            ("max Irms", max(row_by_frame.values(), key=lambda r: r["irms_a"]), "#d04a02"),
            ("max kW", max(row_by_frame.values(), key=lambda r: r["real_power_w"]), "#6a3fb5"),
            ("lowest PF", min(row_by_frame.values(), key=lambda r: r["power_factor"]), "#b00020"),
        ]
